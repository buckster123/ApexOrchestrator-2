...
import asyncio  # For async offload
...
xai_offload_code(model, task, context): Offload code tasks to a specialized xAI model instance; returns report JSON stored in sandbox/offload_reports.json.
...
# New Offload Tool - Async xAI API Call
async def xai_offload_code_async(model: str, task: dict, context: str = "") -> str:
    """Async offload to xAI API; stores report in sandbox/offload_reports.json."""
    report_id = str(uuid.uuid4())
    offload_file = os.path.join(SANDBOX_DIR, "offload_reports.json")
    reports = {}
    if os.path.exists(offload_file):
        with open(offload_file, 'r') as f:
            reports = json.load(f)
    reports[report_id] = {"status": "pending", "model": model, "task": task, "context": context, "timestamp": datetime.now().isoformat()}
    with open(offload_file, 'w') as f:
        json.dump(reports, f)
    try:
        client = OpenAI(api_key=API_KEY, base_url="https://api.x.ai/v1/")
        # Code-specialized prompt
        offload_prompt = f"""You are Code-Grok, a specialized coding assistant. Execute the task: {json.dumps(task)}. Use context: {context}. Respond with a JSON report: {{"output": "...", "confidence": 0.9, "metrics": {{}}}}. Use ReAct for code tasks."""
        response = await asyncio.get_event_loop().run_in_executor(
            None, lambda: client.chat.completions.create(
                model=model,
                messages=[{"role": "system", "content": offload_prompt}],
                stream=False
            )
        )
        output = response.choices[0].message.content
        reports[report_id]["status"] = "completed"
        reports[report_id]["output"] = output
        with open(offload_file, 'w') as f:
            json.dump(reports, f)
        return f"Offload completed. Report ID: {report_id}. Read via fs_read_file('offload_reports.json')."
    except Exception as e:
...
def xai_offload_code(model: str, task: dict, context: str = "") -> str:
    """Sync wrapper for async offload."""
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    return loop.run_until_complete(xai_offload_code_async(model, task, context))

# Tool Schema for Structured Outputs - Updated with new tool
TOOLS = [
    {
        "type": "function",
        "function": {
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "model": {"type": "string", "description": "xAI model to use (e.g., 'grok-code-fast')."},
...
# API Wrapper with Streaming and Tool Handling - With batch commit and safe args, async offload support
            ...
            for func_name, calls in tool_batches.items():
                for tool_call in calls:
                    try:
                        ...
                        elif func_name == "xai_offload_code":
                            model_off = args.get('model', 'grok-code-fast')
                            task = args.get('task', {})
                            context_snip = args.get('context', '')
                            result = xai_offload_code(model_off, task, context_snip)
                        else:
                            result = "Unknown tool."
                    except Exception as e:
